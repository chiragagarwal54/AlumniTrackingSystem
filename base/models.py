...
def upload_news_image_location(instance, filename):
    file_path = "news/{news_id}/{filename}".format(
        news_id=str(instance.id), filename=filename
    )
    return file_path
...
def upload_notice_image_location(instance, filename):
    file_path = "notice/{notice_id}/{filename}".format(
        notice_id=str(instance.id), filename=filename
    )
    return file_path
...
def upload_gallery_image_location(instance, filename):
    file_path = "gallery/{gallery_id}/{filename}".format(
        gallery_id=str(instance.id), filename=filename
    )
    return file_path
...
def upload_carousel_image_location(instance, filename):
    file_path = "carousel/{carousel_id}/{filename}".format(
        carousel_id=str(instance.id), filename=filename
    )
    return file_path
...
class Event(models.Model):
    ...
    image = models.ImageField(
        upload_to=upload_event_image_location, null=True, blank=True
    )

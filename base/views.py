...
def home(request):
    ...
    if not user.is_authenticated or (user.is_authenticated and not user.college):
        ...
        jobs = Job.objects.filter(college=user.college or not college).order_by(
            "-date_created"
        )
        context["jobsitem"] = jobs[0:2]
        context["lastjob"] = jobs.first()
        ...
        donation = DonationType.objects.filter(
            college=user.college or not college
        ).order_by("-date_time")
        context["donation"] = donation[0:2]

        galleryimgs = Gallery.objects.filter(
            college=user.college or not college
        ).order_by("-date_time")
        context["gallery"] = galleryimgs[0:6]

        carousel_images = Carousel.objects.filter(
            college=user.college or not college
        ).order_by("-date_time")
        context["carouselimages"] = carousel_images[0:6]
...
def allevents(request):
    ...
    if not user.is_authenticated or (user.is_authenticated and not user.college):
        ...
        today = datetime.datetime.today()
        upcoming_events = Event.objects.filter(Q(start_date__gte=today)).filter(
            college=user.college
        )
        past_events = Event.objects.filter(Q(start_date__lt=today)).filter(
            college=user.college
        )
        context["upcoming_events"] = upcoming_events
        context["past_events"] = past_events
